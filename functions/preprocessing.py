...
def History(path_db, symbol, start, end):
    ...
    connection.close()
    index_start, index_end = df.index[df['date']==start][0], df.index[df['date']==end][0]
...
def process_data(history, longueur, echantillon, input_dim, target):
    ...
    dates_set = np.array([history['date'].iloc[i] for i in range(0, len(history) - longueur, longueur)])
